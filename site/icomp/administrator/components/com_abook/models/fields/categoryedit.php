...
class JFormFieldCategoryEdit extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		if ($this->element['parent'] || $jinput->get('view') == 'category')
		{
			$oldCat = $jinput->get('id', 0);
			$oldParent = $this->form->getValue($name, 0);
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('extension', 'com_abook');
		}
		else
			// For items the old category is the category they are in when opened or 0 if new.
		{
			$oldCat = $this->form->getValue($name, 0);
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('option', 'com_abook');
		}

		$db = JFactory::getDbo();
		$query = $db->getQuery(true)
			->select('a.id AS value, a.title AS text, a.level, a.published')
			->from('#__abcategories AS a')
			->join('LEFT', $db->quoteName('#__abcategories') . ' AS b ON a.lft > b.lft AND a.rgt < b.rgt');

		// Filter by the extension type
		if ($this->element['parent'] == true || $jinput->get('view') == 'categories')
		{
		...
		if ($oldCat != 0 && ($this->element['parent'] == true || $jinput->get('view') == 'category'))
		{
			// Prevent parenting to children of this item.
			// To rearrange parents and children move the children up, not the parents down.
			$query->join('LEFT', $db->quoteName('#__abcategories') . ' AS p ON p.id = ' . (int) $oldCat)
		...
		{
			// Translate ROOT
			if ($this->element['parent'] == true || $jinput->get('view') == 'category')
			{
