 ...
 * @since	1.5
 */
function AbookBuildRoute(&$query)
{
	$segments	= array();
	...
	if (empty($query['Itemid'])) {
        ...
        // check again
        if ($menuItemGiven && isset($menuItem) && $menuItem->component != 'com_abook')
        {
                $menuItemGiven = false;
                unset($query['Itemid']);
        }
	...
	if ($view == 'author') {
		...
		if ($menuItem->query['view']!='authors'){
			$segments[] =$query['view'];
		}
 ...
 * @since	1.5
 */
function AbookParseRoute($segments)
{
//error_log(print_r($segments, true), 3, '/tmp/debug.log');
	$vars = array();
	...
	if ($count == 1) {
		...
		if ($category && $category->alias == $alias) {
			...
			$query = 'SELECT alias, catid FROM #__abbook WHERE id = '.(int)$id;
			$db->setQuery($query);
	...
	foreach($segments as $segment)
	{
		...
		if ($found == 0) {
			if ($advanced) {
				...
				$query->select("id");
				$query->from("#__abbook");
				$query->where('catid = '.$vars['catid'].' AND alias = '.$db->quote($segment));
				$db->setQuery($query);
				$cid = $db->loadResult();
			} else {
				$cid = $segment;
