...
abstract class JHtmlCategoryparent
{
	/**
	 ...
	 * @return  array
	 */
	public static function options($extension, $config = array('filter.published' => array(0,1)), $cats=0, $level=0)
	{
		...
		if (!isset(self::$items[$hash])) {
			...
			if ($cats>0){
                        	$query->where('b.id ='.$cats);
                	}
			// Filter on the level.
			if ($level!= -1){
                        //	$query->where('a.level <= b.level + '.(int) $level);
                	}
			$query->group('a.id');
			$query->order('a.lft');
			$db->setQuery($query);
			...
			self::$items[$hash][] = JHtml::_('select.option', $cats, JText::_('JLIB_FORM_BUTTON_SELECT'));
