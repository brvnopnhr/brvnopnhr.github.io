...
class JFormFieldModal_Book extends JFormField
{
	...
	protected function getInput()
	{
		...
		// Load language
                JFactory::getLanguage()->load('com_abook', JPATH_ADMINISTRATOR);
		...
		// Build the script.
		$script = array();
		$script[] = '	function jSelectBook_'.$this->id.'(id, title, catid, object) {';
		...
		$html	= array();
		$link	= 'index.php?option=com_abook&amp;view=books&amp;layout=modal&amp;tmpl=component&amp;function=jSelectBook_'.$this->id;
		...
		if (empty($title)) {
			$title = JText::_('COM_ABOOK_SELECT_A_BOOK');
		}
		...
		if (0 == (int)$this->value) {
			...
			$value = (int)$this->value;
                ...
                $html[] = '<a class="modal btn hasTooltip" title="'.JHtml::tooltipText('COM_CONTENT_CHANGE_ARTICLE').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-book"></i> '.JText::_('JSELECT').'</a>';
